...
interface RoundChairRenderProps {
...
function renderBenchSeats(
  ...
  const idPrefix = `roundchair${table.tableNumber}`;
  let seatIndex = 1;
...
const RoundChairRender: React.FC<RoundChairRenderProps> = ({
  table,
  index,
  selected,
  guests,
  ...
  const createSeat = useCallback((x: number, y: number, id: string, guest?: Guest) => (
    <div
      key={id}
      id={id}
      className="seat my-element absolute border border-gray-400 rounded-full text-xs flex items-center justify-center"
      style={{
        top: 1,
        left: 18,
    ...
    >
      ...
      >
         ...
         {(guest && guest.isView) ? (
          ...
          id.replace(`roundchair${table.tableNumber}_`, '')
        )}
      </div>
    </div>
...
export default RoundChairRender;